...
static void processObject( Obj *obj, Scene *scene, mmap& materials )
{
	...
	if( name == "directional_light" ) {
		...
		scene->add( new PointLight( scene, 
			...
			hasField(child, "constant_attenuation_coeff") ? getField(child, "linear_attenuation_coeff")->getScalar() : 0,
			hasField(child, "constant_attenuation_coeff") ? getField(child, "quadratic_attenuation_coeff")->getScalar() : 0));
	} else if( 	name == "sphere" ||
				name == "box" ||
				name == "cylinder" ||
				name == "cone" ||
