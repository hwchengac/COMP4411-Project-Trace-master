...
{
	((TraceUI*)(o->user_data()))->m_nQuadAtten = double(((Fl_Slider*)o)->value());
}
...
double TraceUI::getConstAtten() {
	return m_nConstAtten;
}

double TraceUI::getLinearAtten() {
	return m_nLinearAtten;
}

double TraceUI::getQuadAtten() {
	return m_nQuadAtten;
}
...
TraceUI::TraceUI() {
	...
	m_mainWindow = new Fl_Window(100, 40, 350, 480, "Ray <Not Loaded>");
		...
		m_linearAttenSlider->minimum(0.0);
		m_linearAttenSlider->maximum(1.0);
